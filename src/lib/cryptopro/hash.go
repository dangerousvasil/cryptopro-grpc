...
func CreateCryptHash(hProv *CryptoProv, algo uint) (*CryptoHash, error) {
	...
	if status == 0 {
		...
		return nil, fmt.Errorf("can't get hash got error %s", lastError)
	}
...
func (hHash CryptoHash) CryptHashData(data []byte) error {
	...
	if status == 0 {
		return fmt.Errorf("can't hash data got error %s", GetLastError())
	}
	return nil
}
...
func (hHash CryptoHash) CryptGetHashParam() ([]byte, error) {
	...
	if status == 0 {
		return nil, fmt.Errorf("can't get hash size got error %s", GetLastError())
	}

	value := make([]byte, size)
	status = C.CryptGetHashParam(*hHash.hHash, HP_HASHVAL, (*C.uchar)(&value[0]), &size, 0)
	if status == 0 {
		return nil, fmt.Errorf("can't get hash value got error %s", GetLastError())
	}
...
func (hHash CryptoHash) DestoryHash() error {
	...
	if status == 0 {
		return fmt.Errorf("can't destroy hash got error %s", GetLastError())
	}
	return nil
}
