...
const (
	...
	CERT_STORE_ADD_USE_EXISTING = C.CERT_STORE_ADD_USE_EXISTING
)
...
func CertGetSubjectCertificateFromStore(store *CertStore, data []byte) (*CertContext, error) {
	...
	if pCert == nil {
		return nil, fmt.Errorf("can't get subject certificate from store got error %s", GetLastError())
	}
...
func CertFreeCertificateContext(cert *CertContext) error {
	...
	if status == 0 {
		return fmt.Errorf("can't free cert context got error %s", GetLastError())
	}
	return nil
}
