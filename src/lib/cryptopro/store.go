...
func CertOpenSystemStore(storeName string) (*CertStore, error) {
	...
	if store == nil {
		return nil, fmt.Errorf("can`t open store %s got error %s", storeName, GetLastError())
	}
...
func CertMsgOpenStore(msg *CryptMsg) (*CertStore, error) {
	...
	if store == nil {
		return nil, fmt.Errorf("can't open store for message got error %s", GetLastError())
	}
	return &CertStore{HCertStore: &store}, nil
}
...
func CertCloseStore(store *CertStore, flags uint32) error {
	...
	if status == 0 {
		return fmt.Errorf("can't close store got error %s", GetLastError())
	}
	return nil
}
