...
func CryptAquireCertificatePrivateKey(context *CertContext) (*CryptoProv, error) {
	...
	if status == 0 {
		return nil, fmt.Errorf("can't acquire private key got error %s", GetLastError())
	}
...
func (k *Key) CryptGetKeyParam(paramType uint) (uint, error) {
	...
	if status == 0 {
		return 0, fmt.Errorf("can't acquire param from key got error %s", GetLastError())
	}
...
func CryptGenKey(cryptoProv *CryptoProv, algo uint, flags uint) (*C.HCRYPTKEY, error) {
	...
	if status == 0 {
		return nil, fmt.Errorf("can't generate key get error %s", GetLastError())
	}
...
func CryptGetUserKey(prov *CryptoProv, keySpec uint) (*Key, error) {
	...
	if status == 0 {
		return nil, fmt.Errorf("can't get key got error %s", GetLastError())
	}
...
func CryptExportKey(key *Key, blobType uint) ([]byte, error) {
	...
	if status == 0 {
		return nil, fmt.Errorf("can't export keypubKey gpt error %s", GetLastError())
	}

	blob := make([]byte, size)
	status = C.CryptExportKey(*key.hCryptKey, 0, C.uint(blobType), 0, (*C.uchar)(&blob[0]), &size)
	if status == 0 {
		return nil, fmt.Errorf("can't export keypubKey got error %s", GetLastError())
	}
...
func CryptImportKey(prov *CryptoProv, keyBlob []byte) (*Key, error) {
	...
	if status == 0 {
		return nil, fmt.Errorf("can't import key got error %s", GetLastError())
	}
